...
import { View, Text, Image, StyleSheet, TouchableOpacity, TextInput, ScrollView, Button, Alert } from 'react-native';
...
export default function TravelInfo({ route }: any) {
    ...
    const [hours, minutes] = travelData.time.split(':');
    currentDate.setHours(parseInt(hours, 10));
    currentDate.setMinutes(parseInt(minutes, 10));    
    const [tempTime, setTempTime] = useState(currentDate.toISOString().toString());
    ...
    const showTimePicker = () => {
        if (travelData.title === formattedDate){
            setMinimunTime(new Date().toDateString());
        }
        else {
            setMinimunTime("");
        }      
    ...
    const fetchTravelData = async (travelId: string) => {
        try {
    ...
    const handleInputChange = (field: any, value: any) => {
        setEditedTravelData((prevData: any) => ({
          ...prevData,
          [field]: value,
        }));
        if (field === 'from') {
          ...
          if (userDocSnapshot.exists()) {
            ...
            if (user.uid === travelData.userId) {
              ...
              const updatedReservedTravels = reservedTravels.filter(
                (reservedTravelId: string) => reservedTravelId !== travelData.travelId
              );
                        ...
                        <View style={styles.contentInfo}>
                            ...
                            {editor ? (
                                ...
                                <DateTimePicker
                                    ...
                                    mode={'time'}
                                    minimumDate={new Date(minimunTime)}
                                />
                            </>
                            ) : (
